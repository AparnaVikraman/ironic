...
http_opts = [
    ...
    cfg.StrOpt('uefi_bootfile_name',
               default=paths.basedir_def(
                   'drivers/modules/grubnetx64.efi.signed'),
...
def _build_http_config(http_options, template, root_tag, disk_ident_tag):
    ...
    """
    params = CONF.deploy.kernel_cmdline_params
    kernel_params = ""
...
def _build_http_boot_script(uuid, script, http_options):
    ...
    :param http_options: A dict of values to set on the boot script.
    :returns: A formatted string with the file content.
    ...
    grub = '/'.join([CONF.deploy.http_url, CONF.deploy.http_root, uuid, 
                     'grub.cfg'])
    bootfile = '/'.join([CONF.deploy.http_url, CONF.deploy.http_root, 
...
def create_http_config(task, http_options, template=None):
    """Generate HTTP configuration file.
...
def create_http_boot_script(task, http_options, script=None):
    """Generate HTTP boot script.
    
    This method will generate the HTTP boot script for the task's node
    under a directory named with the UUID of that node.
    
    :param task: A TaskManager instance.
    :param http_options: A dictionary with the HTTP configuration
    parameters.
    :param script: The HTTP boot script template.
    """
...
def _build_http_config_options(task, http_info):
    ...
    http_options = {
        ...
        'kernel_cmdline_params': CONF.deploy.kernel_cmdline_params,
...
def switch_http_boot_script(path, http_options):
    """Switch a http boot script from deployment mode to service mode.
    
    :param path: path to the http config file in httpboot.
    :param http_info: a dict of values to set on the boot script.
    """
    pattern = '^set -v kernel .*$'
    boot_line = 'set -v kernel %s' % http_options['aki_path']

    deploy_utils.replace_lines_in_file(path, pattern, boot_line)

    pattern = '^set -v ramdisk .*$'
    boot_line = 'set -v ramdisk %s' % http_options['ari_path']

    deploy_utils.replace_lines_in_file(path, pattern, boot_line)

def switch_http_config(path, root_uuid):
    """Switch a http config from deployment mode to service mode.
    
    :param path: path to the http config file in httpboot.
    :param root_uuid: root uuid in case of partition image.
    """
    deploy_utils.replace_root_uuid(path, root_uuid)
...
class HTTPBoot(base.BootInterface):
    ...
    def prepare_ramdisk(self, task, ramdisk_params):
        ...
        bootfile_path = os.path.join( 
            ...
            os.path.basename(CONF.http.uefi_bootfile_name)) 
        shutil.copyfile(CONF.http.uefi_bootfile_name, bootfile_path)
        ...
        if node.provision_state == states.DEPLOYING:
            http_info.update(deploy_utils.get_instance_image_info(
                node, task.context, get_root_dir()))
        ...
        boot_url = os.path.join(CONF.deploy.http_url, boot_script_path)
    ...
    def prepare_instance(self, task):
        ...
        instance_image_info = deploy_utils.get_instance_image_info(
            task.node, task.context, get_root_dir())
    ...
    def clean_up_instance(self, task):
        ...
        node = task.node
        try:
            images_info = deploy_utils.get_instance_image_info(
                node, task.context, get_root_dir())
        except exception.MissingParameterValue as e:
